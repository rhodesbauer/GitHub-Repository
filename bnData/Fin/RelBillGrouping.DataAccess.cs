...
{
	public class RelBillGroupingDataAccess : BaseDataAccess
	{
		#region Properties
		
		private RelBillGrouping _entlocal;
		private RelBillGrouping entLocal
		{
			get { if (_entlocal == null) _entlocal = new RelBillGrouping(); return _entlocal; }
			set { _entlocal = value; }
		}
		#endregion
		...
		public RelBillGroupingDataAccess () 
		{}
		public RelBillGroupingDataAccess ( Context _context ) :base(_context)
		{}
		#endregion
		...
		{
			entLocal = (RelBillGrouping)ValidateData(base.Save(entToSave));
		...
		{
			entLocal = (RelBillGrouping)ValidateData(base.Update(entToUpdate));
			if (lstValidations.Count > 0)
			{
				string strValidations = "";
				foreach(var item in lstValidations)
			 ...
			 else
			 {
				entLocal.userIDLastChange = Context.loggedUser.UserId;
				entLocal.dtLastChange = DateTime.Now;
				Context.RelBillGrouping.Update(entLocal);
				Context.SaveChanges();
			}
			return entLocal;
		}
		...
		{
			entLocal = (RelBillGrouping)entToDelete;
			...
			{
			 	isDeletable = Context.RelBillGrouping.Remove(entLocal).State == EntityState.Deleted;
			}
			return isDeletable;
		}
		...
		public  IEntity Get ( IEntity entToSearch ) 
		{
			if (entToSearch.GetType().Equals(entLocal.GetType())){
				if (!string.IsNullOrEmpty(((RelBillGrouping)entToSearch).ConId))
					return Get(((RelBillGrouping)entToSearch).ConId);
				else {
					entLocal = (RelBillGrouping)entToSearch;
					if (entLocal.ConCode > 0)
						return Context.RelBillGrouping.Where(x=> x.ConCode.Equals(entLocal.ConCode)).FirstOrDefault();
					if (!string.IsNullOrEmpty(entLocal.ConFirstName) && !string.IsNullOrEmpty(entLocal.ConLastname))
						return Context.RelBillGrouping.Where(x=> x.ConLastname.Equals(entLocal.ConLastname) && x.ConFirstName.Equals(entLocal.ConFirstName)).FirstOrDefault();
				}
			}
			return null;
		}
