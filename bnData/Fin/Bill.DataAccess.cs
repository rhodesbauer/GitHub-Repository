...
namespace Data.Fin
{
	public class BillDataAccess : BaseDataAccess
	{
		...
		private EntBill _entlocal;
		private EntBill entLocal
		{
			get { if (_entlocal == null) _entlocal = new EntBill(); return _entlocal; }
			set { _entlocal = value; }
		}
		#endregion
		...
		{}
		public BillDataAccess ( Context _context ) :base(_context)
		{}
		#endregion
		...
		{
			entLocal = (Bill)ValidateData(base.Save(entToSave));
		...
		{
			entLocal = (Bill)ValidateData(base.Update(entToUpdate));
			if (lstValidations.Count > 0)
			{
				string strValidations = "";
				foreach(var item in lstValidations)
		...
		public override bool Delete ( IEntity entToDelete ) 
		{
			...
			{
			 	isDeletable = Context.Bill.Remove(entLocal).State == EntityState.Deleted;
			}
			return isDeletable;
		}
		...
		public  IEntity Get ( IEntity entToSearch ) 
		{
			if (entToSearch.GetType().Equals(entLocal.GetType())){
				if (!string.IsNullOrEmpty(((Bill)entToSearch).ConId))
					return Get(((Bill)entToSearch).ConId);
				else {
					entLocal = (Bill)entToSearch;
					if (entLocal.ConCode > 0)
						return Context.Bill.Where(x=> x.ConCode.Equals(entLocal.ConCode)).FirstOrDefault();
					if (!string.IsNullOrEmpty(entLocal.ConFirstName) && !string.IsNullOrEmpty(entLocal.ConLastname))
						return Context.Bill.Where(x=> x.ConLastname.Equals(entLocal.ConLastname) && x.ConFirstName.Equals(entLocal.ConFirstName)).FirstOrDefault();
				}
			}
			return null;
		}
