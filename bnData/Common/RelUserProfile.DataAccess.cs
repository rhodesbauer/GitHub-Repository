...
{
	public class RelUserProfileDataAccess : BaseDataAccess
	{
		#region Properties
		
		private RelUserProfiles _entlocal;
		private RelUserProfiles entLocal
		{
			get { if (_entlocal == null) _entlocal = new RelUserProfiles(); return _entlocal; }
			set { _entlocal = value; }
		}
		#endregion
		...
		public RelUserProfileDataAccess () 
		{}
		public RelUserProfileDataAccess ( Context _context ) :base(_context)
		{}
		#endregion
		...
		{
			entLocal = (RelUserProfile)ValidateData(base.Save(entToSave));
		...
		{
			entLocal = (RelUserProfile)ValidateData(base.Update(entToUpdate));
			if (lstValidations.Count > 0)
			{
				string strValidations = "";
				foreach(var item in lstValidations)
			 ...
			 else
			 {
				entLocal.userIDLastChange = Context.loggedUser.UserId;
				entLocal.dtLastChange = DateTime.Now;
				Context.RelUserProfile.Update(entLocal);
				Context.SaveChanges();
			}
			return entLocal;
		}
		...
		{
			entLocal = (RelUserProfile)entToDelete;
			...
			{
			 	isDeletable = Context.RelUserProfile.Remove(entLocal).State == EntityState.Deleted;
			}
			return isDeletable;
		}
		...
		public  IEntity Get ( IEntity entToSearch ) 
		{
			if (entToSearch.GetType().Equals(entLocal.GetType())){
				if (!string.IsNullOrEmpty(((RelUserProfile)entToSearch).ConId))
					return Get(((RelUserProfile)entToSearch).ConId);
				else {
					entLocal = (RelUserProfile)entToSearch;
					if (entLocal.ConCode > 0)
						return Context.RelUserProfile.Where(x=> x.ConCode.Equals(entLocal.ConCode)).FirstOrDefault();
					if (!string.IsNullOrEmpty(entLocal.ConFirstName) && !string.IsNullOrEmpty(entLocal.ConLastname))
						return Context.RelUserProfile.Where(x=> x.ConLastname.Equals(entLocal.ConLastname) && x.ConFirstName.Equals(entLocal.ConFirstName)).FirstOrDefault();
				}
			}
			return null;
		}
