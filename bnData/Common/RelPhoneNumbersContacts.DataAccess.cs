...
{
	public class RelPhoneNumbersContactDataAccess : BaseDataAccess
	{
		#region Properties
		
		private RelPhoneNumbersContacts _entlocal;
		private RelPhoneNumbersContacts entLocal
		{
			get { if (_entlocal == null) _entlocal = new RelPhoneNumbersContacts(); return _entlocal; }
			set { _entlocal = value; }
		}
		#endregion
		...
		public RelPhoneNumbersContactDataAccess () 
		{}
		public RelPhoneNumbersContactDataAccess ( Context _context ) :base(_context)
		{}
		#endregion
		...
		{
			entLocal = (RelPhoneNumbersContact)ValidateData(base.Save(entToSave));
		...
		{
			entLocal = (RelPhoneNumbersContact)ValidateData(base.Update(entToUpdate));
			if (lstValidations.Count > 0)
			{
				string strValidations = "";
				foreach(var item in lstValidations)
			 ...
			 else
			 {
				entLocal.userIDLastChange = Context.loggedUser.UserId;
				entLocal.dtLastChange = DateTime.Now;
				Context.RelPhoneNumbersContact.Update(entLocal);
				Context.SaveChanges();
			}
			return entLocal;
		}
		...
		{
			entLocal = (RelPhoneNumbersContact)entToDelete;
			...
			{
			 	isDeletable = Context.RelPhoneNumbersContact.Remove(entLocal).State == EntityState.Deleted;
			}
			return isDeletable;
		}
		...
		public  IEntity Get ( IEntity entToSearch ) 
		{
			if (entToSearch.GetType().Equals(entLocal.GetType())){
				if (!string.IsNullOrEmpty(((RelPhoneNumbersContact)entToSearch).ConId))
					return Get(((RelPhoneNumbersContact)entToSearch).ConId);
				else {
					entLocal = (RelPhoneNumbersContact)entToSearch;
					if (entLocal.ConCode > 0)
						return Context.RelPhoneNumbersContact.Where(x=> x.ConCode.Equals(entLocal.ConCode)).FirstOrDefault();
					if (!string.IsNullOrEmpty(entLocal.ConFirstName) && !string.IsNullOrEmpty(entLocal.ConLastname))
						return Context.RelPhoneNumbersContact.Where(x=> x.ConLastname.Equals(entLocal.ConLastname) && x.ConFirstName.Equals(entLocal.ConFirstName)).FirstOrDefault();
				}
			}
			return null;
		}
