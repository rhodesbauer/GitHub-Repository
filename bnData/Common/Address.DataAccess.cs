...
{
	public class AddressDataAccess : BaseDataAccess
	{
		#region Properties
		
		private EntAddresses _entlocal;
		private EntAddresses entLocal
		{
			get { if (_entlocal == null) _entlocal = new EntAddresses(); return _entlocal; }
			set { _entlocal = value; }
		}
		#endregion
		...
		{}
		public AddressDataAccess ( Context _context ) :base(_context)
		{}
		#endregion
		...
		{
			entLocal = (Addresse)ValidateData(base.Save(entToSave));
		...
		{
			entLocal = (Addresse)ValidateData(base.Update(entToUpdate));
			if (lstValidations.Count > 0)
			{
				string strValidations = "";
				foreach(var item in lstValidations)
			 ...
			 else
			 {
				entLocal.userIDLastChange = Context.loggedUser.UserId;
				entLocal.dtLastChange = DateTime.Now;
				Context.Addresse.Update(entLocal);
				Context.SaveChanges();
			}
			return entLocal;
		}
		...
		public override bool Delete ( IEntity entToDelete ) 
		{
			...
			{
			 	isDeletable = Context.Addresse.Remove(entLocal).State == EntityState.Deleted;
			}
			return isDeletable;
		}
		...
		public  IEntity Get ( IEntity entToSearch ) 
		{
			if (entToSearch.GetType().Equals(entLocal.GetType())){
				if (!string.IsNullOrEmpty(((Addresse)entToSearch).ConId))
					return Get(((Addresse)entToSearch).ConId);
				else {
					entLocal = (Addresse)entToSearch;
					if (entLocal.ConCode > 0)
						return Context.Addresse.Where(x=> x.ConCode.Equals(entLocal.ConCode)).FirstOrDefault();
					if (!string.IsNullOrEmpty(entLocal.ConFirstName) && !string.IsNullOrEmpty(entLocal.ConLastname))
						return Context.Addresse.Where(x=> x.ConLastname.Equals(entLocal.ConLastname) && x.ConFirstName.Equals(entLocal.ConFirstName)).FirstOrDefault();
				}
			}
			return null;
		}
