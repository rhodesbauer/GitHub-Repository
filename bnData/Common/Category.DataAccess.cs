...
{
	public class CategorieDataAccess : BaseDataAccess
	{
		#region Properties
		
		private EntCategories _entlocal;
		private EntCategories entLocal
		{
			get { if (_entlocal == null) _entlocal = new EntCategories(); return _entlocal; }
			set { _entlocal = value; }
		}
		#endregion
		...
		public CategorieDataAccess () 
		{}
		public CategorieDataAccess ( Context _context ) :base(_context)
		{}
		#endregion
		...
		{
			entLocal = (Categorie)ValidateData(base.Save(entToSave));
		...
		{
			entLocal = (Categorie)ValidateData(base.Update(entToUpdate));
			if (lstValidations.Count > 0)
			{
				string strValidations = "";
				foreach(var item in lstValidations)
			 ...
			 else
			 {
				entLocal.userIDLastChange = Context.loggedUser.UserId;
				entLocal.dtLastChange = DateTime.Now;
				Context.Categorie.Update(entLocal);
				Context.SaveChanges();
			}
			return entLocal;
		}
		...
		{
			entLocal = (Categorie)entToDelete;
			...
			{
			 	isDeletable = Context.Categorie.Remove(entLocal).State == EntityState.Deleted;
			}
			return isDeletable;
		}
		...
		public  IEntity Get ( IEntity entToSearch ) 
		{
			if (entToSearch.GetType().Equals(entLocal.GetType())){
				if (!string.IsNullOrEmpty(((Categorie)entToSearch).ConId))
					return Get(((Categorie)entToSearch).ConId);
				else {
					entLocal = (Categorie)entToSearch;
					if (entLocal.ConCode > 0)
						return Context.Categorie.Where(x=> x.ConCode.Equals(entLocal.ConCode)).FirstOrDefault();
					if (!string.IsNullOrEmpty(entLocal.ConFirstName) && !string.IsNullOrEmpty(entLocal.ConLastname))
						return Context.Categorie.Where(x=> x.ConLastname.Equals(entLocal.ConLastname) && x.ConFirstName.Equals(entLocal.ConFirstName)).FirstOrDefault();
				}
			}
			return null;
		}
