...
{
	public class RelContactsAddresseDataAccess : BaseDataAccess
	{
		#region Properties
		
		private RelContactsAddresses _entlocal;
		private RelContactsAddresses entLocal
		{
			get { if (_entlocal == null) _entlocal = new RelContactsAddresses(); return _entlocal; }
			set { _entlocal = value; }
		}
		#endregion
		...
		public RelContactsAddresseDataAccess () 
		{}
		public RelContactsAddresseDataAccess ( Context _context ) :base(_context)
		{}
		#endregion
		...
		{
			entLocal = (RelContactsAddresse)ValidateData(base.Save(entToSave));
		...
		{
			entLocal = (RelContactsAddresse)ValidateData(base.Update(entToUpdate));
			if (lstValidations.Count > 0)
			{
				string strValidations = "";
				foreach(var item in lstValidations)
			 ...
			 else
			 {
				entLocal.userIDLastChange = Context.loggedUser.UserId;
				entLocal.dtLastChange = DateTime.Now;
				Context.RelContactsAddresse.Update(entLocal);
				Context.SaveChanges();
			}
			return entLocal;
		}
		...
		{
			entLocal = (RelContactsAddresse)entToDelete;
			...
			{
			 	isDeletable = Context.RelContactsAddresse.Remove(entLocal).State == EntityState.Deleted;
			}
			return isDeletable;
		}
		...
		public  IEntity Get ( IEntity entToSearch ) 
		{
			if (entToSearch.GetType().Equals(entLocal.GetType())){
				if (!string.IsNullOrEmpty(((RelContactsAddresse)entToSearch).ConId))
					return Get(((RelContactsAddresse)entToSearch).ConId);
				else {
					entLocal = (RelContactsAddresse)entToSearch;
					if (entLocal.ConCode > 0)
						return Context.RelContactsAddresse.Where(x=> x.ConCode.Equals(entLocal.ConCode)).FirstOrDefault();
					if (!string.IsNullOrEmpty(entLocal.ConFirstName) && !string.IsNullOrEmpty(entLocal.ConLastname))
						return Context.RelContactsAddresse.Where(x=> x.ConLastname.Equals(entLocal.ConLastname) && x.ConFirstName.Equals(entLocal.ConFirstName)).FirstOrDefault();
				}
			}
			return null;
		}
