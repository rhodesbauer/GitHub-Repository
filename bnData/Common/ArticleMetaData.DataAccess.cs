...
{
	public class ArticleMetaDataDataAccess : BaseDataAccess
	{
		#region Properties
		
		private EntArticleMetaData _entlocal;
		private EntArticleMetaData entLocal
		{
			get { if (_entlocal == null) _entlocal = new EntArticleMetaData(); return _entlocal; }
			set { _entlocal = value; }
		}
		#endregion
		...
		public ArticleMetaDataDataAccess () 
		{}
		public ArticleMetaDataDataAccess ( Context _context ) :base(_context)
		{}
		#endregion
		...
		{
			entLocal = (ArticleMetaData)ValidateData(base.Save(entToSave));
		...
		{
			entLocal = (ArticleMetaData)ValidateData(base.Update(entToUpdate));
			if (lstValidations.Count > 0)
			{
				string strValidations = "";
				foreach(var item in lstValidations)
			 ...
			 else
			 {
				entLocal.userIDLastChange = Context.loggedUser.UserId;
				entLocal.dtLastChange = DateTime.Now;
				Context.ArticleMetaData.Update(entLocal);
				Context.SaveChanges();
			}
			return entLocal;
		}
		...
		{
			entLocal = (ArticleMetaData)entToDelete;
			...
			{
			 	isDeletable = Context.ArticleMetaData.Remove(entLocal).State == EntityState.Deleted;
			}
			return isDeletable;
		}
		...
		public  IEntity Get ( IEntity entToSearch ) 
		{
			if (entToSearch.GetType().Equals(entLocal.GetType())){
				if (!string.IsNullOrEmpty(((ArticleMetaData)entToSearch).ConId))
					return Get(((ArticleMetaData)entToSearch).ConId);
				else {
					entLocal = (ArticleMetaData)entToSearch;
					if (entLocal.ConCode > 0)
						return Context.ArticleMetaData.Where(x=> x.ConCode.Equals(entLocal.ConCode)).FirstOrDefault();
					if (!string.IsNullOrEmpty(entLocal.ConFirstName) && !string.IsNullOrEmpty(entLocal.ConLastname))
						return Context.ArticleMetaData.Where(x=> x.ConLastname.Equals(entLocal.ConLastname) && x.ConFirstName.Equals(entLocal.ConFirstName)).FirstOrDefault();
				}
			}
			return null;
		}
