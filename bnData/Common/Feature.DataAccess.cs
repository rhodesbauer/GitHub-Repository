...
{
	public class FeatureDataAccess : BaseDataAccess
	{
		#region Properties
		
		private EntFeatures _entlocal;
		private EntFeatures entLocal
		{
			get { if (_entlocal == null) _entlocal = new EntFeatures(); return _entlocal; }
			set { _entlocal = value; }
		}
		#endregion
		...
		{}
		public FeatureDataAccess ( Context _context ) :base(_context)
		{}
		#endregion
		...
		{
			entLocal = (Feature)ValidateData(base.Save(entToSave));
		...
		{
			entLocal = (Feature)ValidateData(base.Update(entToUpdate));
			if (lstValidations.Count > 0)
			{
				string strValidations = "";
				foreach(var item in lstValidations)
			 ...
			 else
			 {
				entLocal.userIDLastChange = Context.loggedUser.UserId;
				entLocal.dtLastChange = DateTime.Now;
				Context.Feature.Update(entLocal);
				Context.SaveChanges();
			}
			return entLocal;
		}
		...
		public override bool Delete ( IEntity entToDelete ) 
		{
			...
			{
			 	isDeletable = Context.Feature.Remove(entLocal).State == EntityState.Deleted;
			}
			return isDeletable;
		}
		...
		public  IEntity Get ( IEntity entToSearch ) 
		{
			if (entToSearch.GetType().Equals(entLocal.GetType())){
				if (!string.IsNullOrEmpty(((Feature)entToSearch).ConId))
					return Get(((Feature)entToSearch).ConId);
				else {
					entLocal = (Feature)entToSearch;
					if (entLocal.ConCode > 0)
						return Context.Feature.Where(x=> x.ConCode.Equals(entLocal.ConCode)).FirstOrDefault();
					if (!string.IsNullOrEmpty(entLocal.ConFirstName) && !string.IsNullOrEmpty(entLocal.ConLastname))
						return Context.Feature.Where(x=> x.ConLastname.Equals(entLocal.ConLastname) && x.ConFirstName.Equals(entLocal.ConFirstName)).FirstOrDefault();
				}
			}
			return null;
		}
