...
{
	public class ArticleDataAccess : BaseDataAccess
	{
		#region Properties
		
		private EntArticles _entlocal;
		private EntArticles entLocal
		{
			get { if (_entlocal == null) _entlocal = new EntArticles(); return _entlocal; }
			set { _entlocal = value; }
		}
		#endregion
		...
		{}
		public ArticleDataAccess ( Context _context ) :base(_context)
		{}
		#endregion
		...
		{
			entLocal = (Article)ValidateData(base.Save(entToSave));
		...
		{
			entLocal = (Article)ValidateData(base.Update(entToUpdate));
			if (lstValidations.Count > 0)
			{
				string strValidations = "";
				foreach(var item in lstValidations)
			 ...
			 else
			 {
				entLocal.userIDLastChange = Context.loggedUser.UserId;
				entLocal.dtLastChange = DateTime.Now;
				Context.Article.Update(entLocal);
				Context.SaveChanges();
			}
			return entLocal;
		}
		...
		public override bool Delete ( IEntity entToDelete ) 
		{
			...
			{
			 	isDeletable = Context.Article.Remove(entLocal).State == EntityState.Deleted;
			}
			return isDeletable;
		}
		...
		public  IEntity Get ( IEntity entToSearch ) 
		{
			if (entToSearch.GetType().Equals(entLocal.GetType())){
				if (!string.IsNullOrEmpty(((Article)entToSearch).ConId))
					return Get(((Article)entToSearch).ConId);
				else {
					entLocal = (Article)entToSearch;
					if (entLocal.ConCode > 0)
						return Context.Article.Where(x=> x.ConCode.Equals(entLocal.ConCode)).FirstOrDefault();
					if (!string.IsNullOrEmpty(entLocal.ConFirstName) && !string.IsNullOrEmpty(entLocal.ConLastname))
						return Context.Article.Where(x=> x.ConLastname.Equals(entLocal.ConLastname) && x.ConFirstName.Equals(entLocal.ConFirstName)).FirstOrDefault();
				}
			}
			return null;
		}
