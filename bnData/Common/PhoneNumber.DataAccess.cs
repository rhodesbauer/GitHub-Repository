...
{
	public class PhoneNumberDataAccess : BaseDataAccess
	{
		#region Properties
		
		private EntPhoneNumbers _entlocal;
		private EntPhoneNumbers entLocal
		{
			get { if (_entlocal == null) _entlocal = new EntPhoneNumbers(); return _entlocal; }
			set { _entlocal = value; }
		}
		#endregion
		...
		public PhoneNumberDataAccess () 
		{}
		public PhoneNumberDataAccess ( Context _context ) :base(_context)
		{}
		#endregion
		...
		{
			entLocal = (PhoneNumber)ValidateData(base.Save(entToSave));
		...
		{
			entLocal = (PhoneNumber)ValidateData(base.Update(entToUpdate));
			if (lstValidations.Count > 0)
			{
				string strValidations = "";
				foreach(var item in lstValidations)
			 ...
			 else
			 {
				entLocal.userIDLastChange = Context.loggedUser.UserId;
				entLocal.dtLastChange = DateTime.Now;
				Context.PhoneNumber.Update(entLocal);
				Context.SaveChanges();
			}
			return entLocal;
		}
		...
		{
			entLocal = (PhoneNumber)entToDelete;
			...
			{
			 	isDeletable = Context.PhoneNumber.Remove(entLocal).State == EntityState.Deleted;
			}
			return isDeletable;
		}
		...
		public  IEntity Get ( IEntity entToSearch ) 
		{
			if (entToSearch.GetType().Equals(entLocal.GetType())){
				if (!string.IsNullOrEmpty(((PhoneNumber)entToSearch).ConId))
					return Get(((PhoneNumber)entToSearch).ConId);
				else {
					entLocal = (PhoneNumber)entToSearch;
					if (entLocal.ConCode > 0)
						return Context.PhoneNumber.Where(x=> x.ConCode.Equals(entLocal.ConCode)).FirstOrDefault();
					if (!string.IsNullOrEmpty(entLocal.ConFirstName) && !string.IsNullOrEmpty(entLocal.ConLastname))
						return Context.PhoneNumber.Where(x=> x.ConLastname.Equals(entLocal.ConLastname) && x.ConFirstName.Equals(entLocal.ConFirstName)).FirstOrDefault();
				}
			}
			return null;
		}
