...
{
	public class RelUserContactDataAccess : BaseDataAccess
	{
		#region Properties
		
		private RelUserContact _entlocal;
		private RelUserContact entLocal
		{
			get { if (_entlocal == null) _entlocal = new RelUserContact(); return _entlocal; }
			set { _entlocal = value; }
		}
		#endregion
		...
		public RelUserContactDataAccess () 
		{}
		public RelUserContactDataAccess ( Context _context ) :base(_context)
		{}
		#endregion
		...
		{
			entLocal = (RelUserContact)ValidateData(base.Save(entToSave));
		...
		{
			entLocal = (RelUserContact)ValidateData(base.Update(entToUpdate));
			if (lstValidations.Count > 0)
			{
				string strValidations = "";
				foreach(var item in lstValidations)
			 ...
			 else
			 {
				entLocal.userIDLastChange = Context.loggedUser.UserId;
				entLocal.dtLastChange = DateTime.Now;
				Context.RelUserContact.Update(entLocal);
				Context.SaveChanges();
			}
			return entLocal;
		}
		...
		{
			entLocal = (RelUserContact)entToDelete;
			...
			{
			 	isDeletable = Context.RelUserContact.Remove(entLocal).State == EntityState.Deleted;
			}
			return isDeletable;
		}
		...
		public  IEntity Get ( IEntity entToSearch ) 
		{
			if (entToSearch.GetType().Equals(entLocal.GetType())){
				if (!string.IsNullOrEmpty(((RelUserContact)entToSearch).ConId))
					return Get(((RelUserContact)entToSearch).ConId);
				else {
					entLocal = (RelUserContact)entToSearch;
					if (entLocal.ConCode > 0)
						return Context.RelUserContact.Where(x=> x.ConCode.Equals(entLocal.ConCode)).FirstOrDefault();
					if (!string.IsNullOrEmpty(entLocal.ConFirstName) && !string.IsNullOrEmpty(entLocal.ConLastname))
						return Context.RelUserContact.Where(x=> x.ConLastname.Equals(entLocal.ConLastname) && x.ConFirstName.Equals(entLocal.ConFirstName)).FirstOrDefault();
				}
			}
			return null;
		}
